...
export class EditProfileComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    ...
    this.dataService.avatar$.subscribe((avatar) => {
      this.avatar = avatar;
    });
    this.dataService.about$.subscribe((about) => {
      this.AboutRefresh = about;
    });
  }
  ...
  public UpdatePersonal() {
    ...
    if (this.images) {
      avatarValue = Common.PATHS.domain + '/static/uploads/' + this.images.name;
      this.SendFile();
    } else {
      avatarValue = '';
    }
    ...
    this.apiService
      ...
      .subscribe((dataResponse: CustomeResponse) => {
        if (dataResponse.status === ApiStatus.SUCCESS) {
          ...
          this.authService.setProfile(dataResponse.data);
  ...
  refresh() {
    ...
    this.OnAboutContent.emit(this.about);
  }
